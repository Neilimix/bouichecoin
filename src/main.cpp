...
#if defined(NDEBUG)
# error "Bouichecoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x27973d85b103d3cd8e66d45bced8779f1b452f21653f2929fa190e706e94cfcd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bouichecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bouichecoin Signed Message:\n";
...
{
    // Bouichecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Bouichecoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10*30; // Bouichecoin: 5 minutes
static const int64 nTargetSpacing = 1*30; // Bouichecoin: 0.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bouichecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bouichecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x27973d85b103d3cd8e66d45bced8779f1b452f21653f2929fa190e706e94cfcd");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "que ce passe il ici";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040184710fa689ad5023690c80f3a42c8f13f8d45b8c857dacbc8bc3a8e1d3eb4b10f4d4604fa08bsu601aaa0a470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1515166793;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1148926;
        ...
        assert(block.hashMerkleRoot == uint256("0x90ec80462b8c7f232ce345358afca123c744b3a630c1f50626128dfc7851529c"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bouichecoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BouichecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BouichecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BouichecoinMiner(CWallet *pwallet)
{
    printf("BouichecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bouichecoin-miner");
    ...
    try { loop {
        ...
        printf("Running BouichecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BouichecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BouichecoinMiner, pwallet));
}
